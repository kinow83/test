...
void w_callback(int fd, short ev, void *arg)
{
	const char *buf = "kakakakakaka";
	write(fd, buf, strlen(buf));
	printf("write\n");
}

void r_callback(int fd, short ev, void *arg)
{
	struct event *w_event = (struct event*)arg;
	printf("read\n");
	char buf[1024];
...
void accept_callback(int fd, short ev, void *arg)
{
	...
	struct event *w_event, *r_event;

	w_event = event_new(base, client_fd, EV_WRITE /*| EV_PERSIST*/, w_callback, NULL);
	r_event = event_new(base, client_fd, EV_READ | EV_PERSIST, r_callback, w_event);
