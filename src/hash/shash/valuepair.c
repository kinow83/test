 ...
 *
 * Version:	$Id: valuepair.c,v 1.2 2006/03/30 01:47:04 cvs Exp $
 *
...
static const BYTE rcsid[] = "$Id: valuepair.c,v 1.2 2006/03/30 01:47:04 cvs Exp $";
...
VALUE_PAIR *paircreate(int attr, int type)
{
	...
	if ((da = dict_attrbyvalue(attr)) != NULL) {
		snprintf(vp->name, sizeof(vp->name), "%s", da->name);
		vp->type = da->type;
		vp->flags = da->flags;
		...
		if (v) {
			snprintf(vp->name, sizeof(vp->name), "%s-Attr-%u",
				v->name, attr & 0xffff);
		} else {
			snprintf(vp->name, sizeof(vp->name), "Vendor-%u-Attr-%u",
...
VALUE_PAIR *pairparsevalue(VALUE_PAIR *vp, const char *value)
{
	...
	switch(vp->type) {
		...
		case PW_TYPE_DATE:
			if (gettime(value, (time_t *)&tval) < 0) {
				...
				int operator)
{
	...
	if (strncasecmp(attribute, "Attr-", 5) == 0) {
		...
		if ((p - attribute) >= (int) sizeof(buffer)) goto error;
...
void debug_value_pair(const char *desc, VALUE_PAIR *vp)
{
	...
	while (vp)
	{
		switch (vp->type) {
		case PW_TYPE_STRING:
			printf("%s(%d) = %s(%d)\n", vp->name, vp->attribute, vp->strvalue, vp->length);
			break;
		default:
			printf("%s(%d) = %u(%d)\n", vp->name, vp->attribute, vp->lvalue, vp->length);
			break;
		}
		vp = vp->next;
	}
...
int str_reg_chack ( char *text, char *pattern )
{
	int cmp_result = -1;
#ifdef HAVE_REGEX_H
	regex_t         reg;

	regcomp(&reg, (char *)pattern, REG_EXTENDED|REG_NOSUB);
	cmp_result = regexec(&reg, (char *)text, 0, NULL, 0);
	regfree(&reg);
	if (cmp_result == 0) {
		return cmp_result;
	} else {
