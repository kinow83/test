 ...
 *
 * Version:	$Id: print.c,v 1.2 2006/03/30 01:47:04 cvs Exp $
 *
...
static const char rcsid[] = "$Id: print.c,v 1.2 2006/03/30 01:47:04 cvs Exp $";
	...
	switch (vp->type) {
		case PW_TYPE_STRING:
			...
			if (vp->attribute == PW_NAS_PORT)
				a = (char *)vp->strvalue;
#if 1 // by syyang
			else if (vp->attribute == PW_CALLING_STATION_ID) {
				if (normalize_mac (vp->strvalue, buf))
		...
		case PW_TYPE_IPV6ADDR:
			a = (char *) ipv6_ntoa(buf, sizeof(buf), vp->strvalue);
			break;
