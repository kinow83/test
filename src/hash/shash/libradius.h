 ...
 *
 * Version:	$Id: libradius.h,v 1.1.1.1 2006/02/24 08:09:17 cvs Exp $
 *
 */
...
#define fr_ip4addr  ipaddr.ip4addr
#define fr_ip6addr  ipaddr.ip6addr
#define fr_ip4addr_addr ipaddr.ip4addr.s_addr
#define fr_ip6addr_addr ipaddr.ip6addr.s6_addr
...
void lrad_hmac_sha1_vector(const unsigned char *text[], int *text_len, int num_elem,
           const unsigned char *key, int key_len,
           unsigned char *digest);
...
VALUE_PAIR	*find_rad_attribute( RADIUS_PACKET *packet , int attr_id ); //added by kinow
...
VALUE_PAIR *pairfindbydict( VALUE_PAIR *, char *);   // added by hazard
...
int 		normalize_mac(const char *mac, char *normal); // by syyang
int 		valid_ipv4address(const char * ip);	// by kinow
int 		valid_ipv6address(const char * ip);
int 		valid_macaddress(const char *mac);	// by kinow
int 		valid_ipaddress(int addrfamily, const char * ip);
...
int fr_ipaddr_debug(const char *method, const char *desc, const fr_ipaddr_t * ipaddr, char *interface);
...
#define CIPHER_SEED128	1
#define CIPHER_SEED256	2
#define CIPHER_AES128	10
#define CIPHER_AES192	11
#define CIPHER_AES256	12
#define CIPHER_AES512	13
#define CIPHER_DES		20
#define CIPHER_DES3		21
...
void debug_value_pair(const char *desc, VALUE_PAIR *vp);
#endif /*LIBRADIUS_H*/
