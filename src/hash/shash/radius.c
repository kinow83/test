 ...
 *
 * Version:	$Id: radius.c,v 1.1.1.1 2006/02/24 08:09:18 cvs Exp $
 *
...
static const char rcsid[] = "$Id: radius.c,v 1.1.1.1 2006/02/24 08:09:18 cvs Exp $";
...
//dst_ipaddr->scope = src_ipaddr->scope = 3;
//dst_port = 1812;
     ...
     */
	int send_bytes = -1;
    send_bytes = sendto(sockfd, data, data_len, flags,
              (struct sockaddr *) &dst, sizeof_dst);
 ...
 *  possible combinations.
 */
static ssize_t rad_recvfrom(int sockfd, uint8_t *data, size_t data_size, int flags,
                ...
                fr_ipaddr_t *dst_ipaddr, uint16_t *dst_port)
{
     ...
     *  a critical problem.
     */
    if (getsockname(sockfd, (struct sockaddr *)&dst, &sizeof_dst) < 0) return -1;
    ...
    if (dst.ss_family == AF_INET || dst.ss_family == AF_INET6) {
        data_len = recvfromto(sockfd, data, data_size, flags,
                 	(struct sockaddr *)&src, &sizeof_src,
                 	(struct sockaddr *)&dst, &sizeof_dst);
    } else
#endif
	{
         ...
         *  No udpfromto, OR an IPv6 socket.  Fail gracefully.
         */
        data_len = recvfrom(sockfd, data, data_size, flags, 
					(struct sockaddr *)&src, &sizeof_src);
	}
    ...
    *src_port = port;
    ...
    if (!fr_sockaddr2ipaddr(&dst, sizeof_dst, dst_ipaddr, &port)) {
		return -1;
	}
 ...
 * ------------------------------------------------
 *
 * @brief   request에 대한 응답 radius 패킷을 전송한다.
 *
 * @note    1.전송할 radius 패킷을 구성한다.
 * @note    2.radius attribute를 구성한다.
 * @note    3.radius reply 패킷을 전송한다.
 *
 * @param   packet: reply 패킷 구조체
 *          original: request 패킷 구조체
			secret: 공유 비밀키
 * @return  응답 패킷 전송 성공 여부
 *			실패: -1
 *			성공: 0이상의 전송한 패킷 바이트 수
 *
 * @date	2011/01/21
 * @author	강석주
 *
	     ...
	     const char *secret)
{
	...
	if (!packet || (packet->sockfd < 0)) {
		cc_func_log("rad_send", "packet, original, secret", "0");
		return 0;
	}
	...
	if (!packet->data) {
		  ...
		  case PW_AUTHENTICATION_REJECT:
              // reject 일때에는 reject 사유를 함께 전송해 준다. PW_REPLY_MESSAGE(18)
              // reject 사유는 original packet의 PW_MODULE_FAILURE_MESSAGE 에 들어있다.
              if (!pairfind(packet->vps, PW_REPLY_MESSAGE)) {
                  VALUE_PAIR *fvp = pairfind(original->vps, PW_MODULE_FAILURE_MESSAGE);
                  if (fvp) {
                      VALUE_PAIR *vp = pairmake("Reply-Message", (char *)fvp->strvalue, T_OP_EQ);
		  ...
		  case PW_ACCESS_CHALLENGE:
			  if (!original) {
				  ...
				  cc_func_log("rad_send", "packet, original, secret", "-1");
				  return -1;
			  }
			  break;
		  ...
		  DEBUG("Sending %s of id %d to %s:%d\n",
				what, packet->id,
				ip_ntoh(&packet->dst_ipaddr, (char *)ip_buffer, sizeof(ip_buffer)),
				packet->dst_port);
	 ...
	 */
	///3.radius reply 패킷을 전송한다.
	send_bytes = rad_sendto(packet->sockfd, packet->data, (int)packet->data_len, 0,
					&packet->src_ipaddr, packet->src_port,
					&packet->dst_ipaddr, packet->dst_port);
	cc_func_log("rad_send", "packet, original, secret", cc_int2char(send_bytes));
	return send_bytes;
}
 ...
 * ------------------------------------------------
 *
 * @brief   request 패킷 인증값(authenticator) 요휴성을 검사한다.
 *
 * @note    1.radius 패킷 구성요소와 NAS의 공유 비밀키의 MD5 해시값으로 유효성을 검사한다.
 *
 * @param   packet: radius 패킷 구조체
 *			secret: 비밀 공유키
 *          
 * @return  요휴성 검사 성공 여부
 *			성공: 2
...
static int calc_acctdigest(RADIUS_PACKET *packet, const char *secret)
{
	...
	if (memcmp(packet->vector, digest, AUTH_VECTOR_LEN) == 0) {
		packet->verified = 1;
		cc_func_log("calc_acctdigest", "packet, secret", "1");
		return 1;
	}

	///1.radius 패킷 구성요소와 NAS의 공유 비밀키의 MD5 해시값으로 유효성을 검사한다.
	/*
	...
	cc_func_log("calc_acctdigest", "packet, secret", cc_int2char(packet->verified));
 ...
 * ------------------------------------------------
 *
 * @brief   radius 패킷을 수신하고 radius 패킷 구조체에 채운다.
 *
 * @note    1.radius 패킷을 수신한다.
 * @note    2.radiusd attribute 유효성을 검사한다.
 *
 * @param   fd: 파일 디스크립터
 *
 * @return  성공: radius 패킷 구조체
 *			실패: NULL
 *
 * @date	2011/01/21
 * @author	강석주
...
RADIUS_PACKET *rad_recv(int fd)
{
	 ...
	 */
	///1.Radius 패킷을 수신한다.
	packet->data_len = rad_recvfrom(fd, data, sizeof(data), sock_flags,
                    &packet->src_ipaddr, &packet->src_port,
                    &packet->dst_ipaddr, &packet->dst_port);
	/*
	...
	if (packet->data_len < AUTH_HDR_LEN) {
		librad_log("WARNING: Malformed RADIUS packet from host %s: too short (received %d < minimum %d)",
			   ip_ntoh(&packet->src_ipaddr, host_ipaddr, sizeof(host_ipaddr)),
	...
	cc_func_log("rad_recv", "fd", "radius packet 구조체");
	return packet;
}
			...
			const char *vector)
{
	...
	unsigned BYTE	digest[AUTH_VECTOR_LEN];
	char*   salt;
	int	i, n, secretlen;
	unsigned len, n2;
		    ...
		    VALUE_PAIR *password)
{
	int		i;
	char		*ptr;
	BYTE		string[MAX_STRING_LEN * 2 + 1];
...
/*
 * return VALUE PAIR from attribute id in radius packet
 * when not found, return NULL
 * added by kinow
 */
VALUE_PAIR * find_rad_attribute(RADIUS_PACKET *packet , int attr_id)
{
	uint32_t		lvalue;
	VALUE_PAIR		*pair;
	uint8_t			*ptr;
		...
		if (attribute == attr_id) {
			...
			case PW_TYPE_STRING:
				if (pair->flags.has_tag) {
					 ...
					 *	then use it.
					 */
					if ((pair->length > 0) && (*ptr < 0x20)) /* TAG_VALID_ZERO(*ptr)) */{
						pair->flags.tag = *ptr;
						pair->length--;
						offset = 1;
					 ...
					 *	pair->length MAY be zero here.
					 */
					memcpy(pair->strvalue, ptr + offset, pair->length);
				} else {
					/*
					 *	Ascend binary attributes never have a
					 *	tag, and neither do the 'octets' type.
			...
			case PW_TYPE_IPADDR:
				...
				if (pair->type == PW_TYPE_INTEGER) {
					DICT_VALUE *dval;
					dval = dict_valbyattr(pair->attribute, pair->lvalue);
					if (dval) {
						strNcpy(pair->strvalue, dval->name,
								sizeof(pair->strvalue));
					}
				}
				break;
 ...
 * get UNIX OS random
 */
int rad_random(unsigned char *buf, size_t len)
{           
    FILE *f;
    size_t rc;
    ...
    if (f == NULL) {
        DEBUG("Could not open /dev/urandom.\n");
        return -1;
    }   
