...
{
        SHA1_CTX context;
        unsigned BYTE k_ipad[65];    /* inner padding -
                                      * key XORd with ipad
                                      */
        unsigned BYTE k_opad[65];    /* outer padding -
                                      * key XORd with opad
                                      */
        unsigned BYTE tk[20];
        int i;
...
void
lrad_hmac_sha1_vector(const unsigned char *text[], int *text_len, int num_elem,
           const unsigned char *key, int key_len,
           unsigned char *digest)
{
        SHA1_CTX context;
		...
		for (i=0; i<num_elem; i++) {
			SHA1Update(&context, text[i], text_len[i]);
...
/*
 *  cc -DTESTING -I ../include/ hmacsha1.c sha1.c -o test -lssl
 */
...
int main(int argc, char **argv)
{
  ...
  int text_len[2];
  text_len[0] = strlen(text[0]);
  text_len[1] = strlen(text[1]);

  int i;

  printf("hmacsha1 testing\n");

  lrad_hmac_sha1_vector(text, text_len, 2, key1, key1_len, digest1);
