...
//	printf("%s\n", buf);
	...
	// last: \n
	rc = mu_header_create(&header, buf, st.st_size+1, NULL);
	free(buf);
	if (rc) {
		exit(1);
	}
	...
	printf("rc=%d, n=%ld, %s\n", rc, n, line);
#else
	while (mu_stream_readline(stream, line, sizeof(line)-1, off, &n) == 0
		&& (n != 0))
	{
		if (n >= 1) {
			if (line[n-1] == '\n')
...
#endif
	mu_stream_destroy(&stream, 0);
	mu_header_destroy(&header, 0);
/*
	mu_header_set_value
